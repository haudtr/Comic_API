...
router.get('/top', async (req, res, next) =>{
  try {
    const data = await MainModel.listItems({},{'task':'top'})
    res.status(200).json(data)
  } catch (error) {
    res.status(400).json({
      success:false
...
router.post('/add', async (req, res, next) => {
  try {
    ...
    params.tacGia = req.body.tacGia
    params.nhanVatChinh = req.body.nhanVatChinh
    params.moTa = req.body.moTa
    params.anhDaiDien = req.body.anhDaiDien
    params.anhBia = req.body.anhBia
    params.luocXem = req.body.luocXem
